<?php
  ...
  foreach ($collections as $key => $value) {
    ...
    foreach ($collections[$key] as $key2 => $value2) {
      ...
      $excel->getActiveSheet()->setCellValue("D$current_row",strtoupper($value2['encoded_by']));
      ...
      $excel->getActiveSheet()->getStyle("B$current_row")->applyFromArray($text_left);
      $excel->getActiveSheet()->getStyle("D$current_row")->applyFromArray($text_left);
      $current_row++;
    }
  }
  ...
  $excel->getActiveSheet()->getStyle("A$current_row")->applyFromArray($text_right);




  $excel->getActiveSheet()->getStyle("A1:E$current_row")->applyFromArray($all_border);
  $excel->getActiveSheet()->getStyle("E$current_row")->applyFromArray($text_center);
  // END OF FOOTER
