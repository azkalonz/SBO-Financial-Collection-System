  ...
  <body>
    ...
    <script type="text/tmpl" id="tmpl">
      ...
      <table class="calendar-table table table-condensed table-tight">
        <thead>
          <tr>
            <td colspan="7" style="text-align: center;border-top: none!important;">
    ...
    (function ($) {
      ...
      function calendar($el, options) {
        ...
        function dayAddEvent(index, event) {
          ...
          var $event = $('<div/>', {'class': 'event', 'style': "cursor:pointer;", 'onclick': event.clickevent, text: event.title, title: event.title, 'data-index': index}),
              start = event.start,
              end = event.end || start,
    ...
    var data = [];
      ...
      for(let i=0; i<events.length; i++){
        if(events[i].endDate==null){
          events[i].endDate = events[i].date;
        }
        let sth = events[i].startTime.split(':')[0];
        let stm = events[i].startTime.split(':')[1];
        let ye = events[i].date.split(', ')[1];
        let mo = getMo(events[i].date.split(', ')[0].split(' ')[0]);
        let da = events[i].date.split(', ')[0].split(' ')[1];

        let eye = events[i].endDate.split(', ')[1];
        let emo = getMo(events[i].endDate.split(', ')[0].split(' ')[0]);
        let eda = events[i].endDate.split(', ')[0].split(' ')[1];

        data.push({
           title: events[i].name,
           start: new Date(ye, mo, da, sth, stm),
           end: new Date(eye, emo, eda, sth, stm),
           allDay: events[i].allDay==1?true:false,
           text: events[i]._desc,
           clickevent: `window.open('/event/?id=${events[i].id}','_parent')`
           });
      }
